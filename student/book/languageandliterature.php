...
$where = "WHERE category='Language & Literature'";
if ($filter === 'available') {
...
if ($filter === 'currently') {
    $where .= " AND EXISTS (SELECT 1 FROM reserve_record WHERE book_id = book.serialNumber AND state='ACTIVE')";
}

// Fetch books with computed availability status considering reservations
...
<head>
  <meta charset="UTF-8" />
  <title>Language & Literature - Library System</title>
  ...
  <style>
    ...
    .banner {
      background-image: url('../../images/book categories/language.jpg');
...
<section class="banner">
  <div class="banner-overlay"><h1>Language & Literature Collection</h1></div>
</section>
...
<div class="container my-4 text-center">
  <?= $feedback ?>
  <a href="languageandliterature.php" class="btn btn-primary <?= $filter === 'all' ? 'active' : '' ?>">All</a>
  <a href="languageandliterature.php?availability=available" class="btn btn-success <?= $filter === 'available' ? 'active' : '' ?>">✓ Available</a>
  <a href="languageandliterature.php?availability=notavailable" class="btn btn-warning <?= $filter === 'notavailable' ? 'active' : '' ?>">✗ Not Available</a>
  <a href="languageandliterature.php?availability=currently" class="btn btn-danger <?= $filter === 'currently' ? 'active' : '' ?>">Currently Unavailable</a>
</div>
