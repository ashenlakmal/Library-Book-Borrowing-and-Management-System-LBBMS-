...
if (isset($_POST['submit'])) {
    ...
    $query = "SELECT password FROM student WHERE Enrollment = '$enrollment'";
    ...
    if (!$row || !password_verify($currentPassword, $row['password'])) {
        ...
        if ($newPassword === $confirmPassword) {
            if (strlen($newPassword) >= 8 && $counter === 4) {
                ...
                $update = "UPDATE student SET password = '$hashed' WHERE Enrollment = '$enrollment'";
                if (mysqli_query($connect, $update)) {
                    echo "<script>alert('Password changed successfully.'); window.location.href='studentprofile.php';</script>";
                } else {
