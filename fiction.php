...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    // Already borrowed?
    $alreadyBorrowed = mysqli_query($connect, "
        SELECT * FROM book_record 
        WHERE student_id='$enrollment' AND book_id='$bookId' AND state='ACTIVE'
    ");
    ...
    if ($action === 'borrow') {
        if (mysqli_num_rows($alreadyBorrowed) > 0) {
            ...
            mysqli_query($connect, "
                INSERT INTO book_record (book_id, student_id, borrow_date, return_date, fines, state)
    ...
    if ($action === 'reserve') {
        $alreadyReserved = mysqli_query($connect, "
            SELECT * FROM reserve_record WHERE student_id='$enrollment' AND book_id='$bookId' AND state='ACTIVE'
        ");
        if (mysqli_num_rows($alreadyBorrowed) > 0) {
            ...
            $row = mysqli_fetch_assoc(mysqli_query($connect, "
                SELECT MIN(return_date) AS next_return FROM book_record WHERE book_id='$bookId'
            "));
            $expected = $row['next_return'] ?? 'Unknown';
            mysqli_query($connect, "
                INSERT INTO reserve_record (student_id, book_id, reservation_date, state)
...
<head>
  ...
  <style>
    ...
    .availability-badge { font-size: 0.8rem; padding: 4px 8px; border-radius: 20px; display: inline-block; color: white; }
...
<section class="banner">
  <div class="banner-overlay"><h1>Fiction Collection</h1></div>
</section>
...
<div class="container my-4 text-center">
  ...
  <a href="fiction.php?availability=notavailable" class="btn btn-warning <?= $filter === 'notavailable' ? 'active' : '' ?>">✗ Not Available</a>
  <a href="fiction.php?availability=currently" class="btn btn-danger <?= $filter === 'currently' ? 'active' : '' ?>">Currently Unavailable</a>
</div>
  ...
  <div class="row g-4">
    <?php while($book = mysqli_fetch_assoc($books)): ?>
      <?php 
        ...
        $label = '';
        if ($status === 'available') { $badgeColor = 'success'; $label = '✓ Available'; }
        elseif ($status === 'not available') { $badgeColor = 'warning'; $label = '✗ Not Available'; }
        elseif ($status === 'currently unavailable') { $badgeColor = 'danger'; $label = 'Currently Unavailable'; }
      ?>
      <div class="col-md-3">
        <div class="card h-100" data-bs-toggle="modal" data-bs-target="#bookModal<?= $book['serialNumber'] ?>">
          <img src="../../uploads/<?= $book['bookphoto'] ?>" class="card-img-top" style="height:250px;object-fit:cover;">
      ...
      <div class="modal fade" id="bookModal<?= $book['serialNumber'] ?>" tabindex="-1">
        <div class="modal-dialog modal-lg modal-dialog-centered">
          <div class="modal-content">
            ...
            <?php
            $alreadyBorrowed = mysqli_query($connect, "
                SELECT * FROM book_record WHERE student_id='$enrollment' AND book_id='{$book['serialNumber']}' AND state='ACTIVE'
            ");
            $alreadyReserved = mysqli_query($connect, "
                SELECT * FROM reserve_record WHERE student_id='$enrollment' AND book_id='{$book['serialNumber']}' AND state='ACTIVE'
            ");
            ?>
            <div class="modal-body row">
              <div class="col-md-5">
                <img src="../../uploads/<?= $book['bookphoto'] ?>" class="img-fluid rounded">
              </div>
              <div class="col-md-7">
                ...
                <?php if ($status === 'available'): ?>
                  <?php if (mysqli_num_rows($alreadyBorrowed) == 0): ?>
                  ...
                  <?php else: ?>
                    <div class="alert alert-info">You have already borrowed this book.</div>
                  <?php endif; ?>
                <?php else: ?>
                  <?php
                  $query = mysqli_query($connect, "
                      SELECT return_date FROM book_record 
                      WHERE book_id='{$book['serialNumber']}' AND state='ACTIVE'
                      ORDER BY borrow_id DESC LIMIT 1
                  ");
                  $next = mysqli_fetch_assoc($query)['return_date'] ?? null;
                  ...
                  <?php if (mysqli_num_rows($alreadyBorrowed) == 0 && mysqli_num_rows($alreadyReserved) == 0): ?>
                    <form method="post">
                      <input type="hidden" name="book_id" value="<?= $book['serialNumber'] ?>">
                      <input type="hidden" name="action" value="reserve">
                      <button class="btn btn-success" type="submit">Reserve Book</button>
                    </form>
                  <?php elseif (mysqli_num_rows($alreadyReserved) > 0): ?>
                    <div class="alert alert-info">You have already reserved this book.</div>
                  <?php endif; ?>
                <?php endif; ?>
              </div>
            </div>
